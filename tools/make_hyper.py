...
def java_works(java_path):
    ...
    try:
        ret = subprocess.run([java_path, "-version"],
...
def find_java_path():
    ...
    if os.path.exists(bundled):
        candidates.append(("bundled tools/jre", bundled))

    java_home = os.environ.get("JAVA_HOME", "")
    if java_home:
        jh_java = os.path.join(java_home, "bin", "java.exe")
        if os.path.exists(jh_java):
            candidates.append(("JAVA_HOME", jh_java))
        ...
        for name in sorted(os.listdir(sp)):
            ...
            if lower.startswith("jdk") or lower.startswith("jre") or lower.startswith("openjdk"):
                ...
                if os.path.exists(candidate):
                    candidates.append((sp, candidate))

    for origin, path in candidates:
        if java_works(path):
            LOG_INFO("Java from " + origin + ": " + path)
            return path
        LOG_INFO("Java at " + path + " is not usable, trying next (" + origin + ")")

    LOG_ERROR("No usable Java runtime found. Install Java or set JAVA_HOME.")
    LOG_ERROR("Note: tools/jre must be a complete JRE (it needs bin/server/jvm.dll).")
    return ""
