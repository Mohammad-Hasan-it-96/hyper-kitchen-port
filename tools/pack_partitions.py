...
# XMAPORT_USE_LEGACY_EROFS = true 时改用 erofs-utils-cygwin 内的旧版 mkfs.erofs 可执行文件
#                            （与上一项不同：上一项换压缩参数，本项换打包工具）
#
import os
import re
import subprocess
...
def call(cmd):
    ...
    try:
        ...
        print(f"    [X] 工具无法启动: {cmd[0]}")
        print(f"        {e}")
        if getattr(e, "winerror", None) == 4551:
            print("        被 Windows 应用控制策略(Smart App Control)拦截。")
            print("        请在 Windows 安全中心关闭 Smart App Control,或右键该 exe > 属性 > 解除锁定。")
        return 1
...
def mkerofs(name, work, work_output, fmt_alg, level, utc=None):
    utc = resolve_utc(utc)
    use_legacy = os.environ.get("XMAPORT_USE_LEGACY_EROFS", "").strip().lower() == "true"
    mkfs = find_tool('mkfs.erofs', legacy=use_legacy)
    if not mkfs:
        print("    [X] mkfs.erofs 未找到")
        return 1
    if use_legacy:
        print(f"    [*] XMAPORT_USE_LEGACY_EROFS=true: 使用旧版 mkfs.erofs (erofs-utils 1.4)")
